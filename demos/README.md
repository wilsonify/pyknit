...
The page content lives in `demos/` and all Python logic stays inside the
`pyknit` Python package (`pyknit/pyscript/`).

- `demos/index.html` — hub page linking to every demo
- `demos/_assets/common.css` — shared styling
- `demos/_wheel/` — the local `pyknit` wheel served to the browser
- `demos/<demo-name>/demo.html` — one static page per demo
- `pyknit/pyscript/_assets/shared.py` — shared Python helpers (DOM access, pyknit feature detection, rendering)
...
2. **Package Installation**: `py-config` automatically downloads:
   ...
   - `pyknit` from the **local wheel** in `demos/_wheel/` (never from PyPI — see above)
