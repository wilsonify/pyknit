...
def _crown_svg(repeats, counts):
    ...
    for i in range(repeats):
        ...
        parts.append(f'<line x1="{cx}" y1="{cy}" x2="{x2:.1f}" y2="{y2:.1f}" stroke="#7b3fa0" stroke-width="2"/>')
...
def _crown_profile_svg(stitches, plan):
    ...
    for i, row in enumerate(plan):
        ...
        parts.append(f'<circle cx="{cx + w:.1f}" cy="{y:.1f}" r="2.6" fill="#7b3fa0"/>')
        parts.append(f'<text x="{cx + w + 5:.1f}" y="{y + 3:.1f}" font-size="8" fill="#5a2a75">{row["after"]}</text>')
    parts.append("</g>")
    parts.append("</svg>")
    return "\n".join(parts)
...
def _sim_plan(repeats, stitches, plan):
    ...
    lines = [
        ...
        f"# {stitches} stitches cast on, {repeats} decrease repeats ({stitches // repeats} per repeat).",
